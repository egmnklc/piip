...
def Command_Line():
    ...
    if optional_answer == "":
        ...
        if os.path.exists(file_name_for_ip_extraction_and_ping) == True:
            valid = set()
            with open(file_name_for_ip_extraction_and_ping) as f_input:
            ...
            #file_ = input("File that conatins ip(s): ")
                ...
                #lines = file_open.read().replace('["','[(').replace(']',')],').replace("['","[('")
                #c = file_open.read().replace(',)',')')
                #lines = lines[: -1]
                #c = lines[len(lines)-1]
                #d = del(c)
                #b = del(b[len(lines)-1])
